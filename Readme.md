...
Der DVBViewerServerApiWrapper ist eine Bibliothek, welche die webbasierte API des 
DVBViewer Media Servers in die .Net Welt holt. Sie wird unter der Verwendung des 
MediaServers von DVBViewer entwickelt.

The DVBViewerServerApiWrapper is a library that brings the web-based API of the 
DVBViewer Media Server into the .Net world. It is developed using DVBViewer's 
MediaServer.

www.dvbviewer.com

### Current state and requirements (Aktueller Status und Voraussetzungen)

- [x] (de) Funktioniert mit dem DVBViewer Media Server 2.0.4.0 und höher.
- [x] (en) Works with the DVBViewer Media Server 2.0.4.0 and higher.
- [x] (de) <u>_Benötigte .Net Framework Version ist 4.7.1!_</u>
- [x] (en) <u>_Required .Net Framework Version is 4.7.1!_</u>

Wrapper Version 0.5.0.0
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            */
            ...
            byte[] keySecure;
            byte[] keyPublic;
            string pwEncrypted = Security.GenerateEnrcyptedPassword("thePasswort", out keyPublic, out keySecure);

            //Save all, also `keySecure`, `keyPublic` and `pwEncrypted` in the same, better in different places
            //Please save the `keySecure` on a place where the user can access but no other person have access!
            //After the App is new or re-started, load the encrypted values (pwEncrypted, keyPublic, keySecure)

            //Decrypt the password with
            string password = Security.GenerateUnEnrcyptedPassword(pwEncrypted, keyPublic, keySecure);
